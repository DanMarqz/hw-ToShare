window.onload = function() {
    ...
    //variables de inicio:
    var marcha=0; //control del temporizador - indica si está corriendo el tiempo o no
    var cro=0; //estado inicial del cronómetro. - indica tiempo transcurrido en ms (milisegundos)
}
//botón Empezar / Reiniciar
function activo (){   
    if (document.cron.boton1.value=="Start") { //botón en "Empezar"
...
function tiempo() { 
    ...
    ho=cr.getHours()-20; //horas 	
    //poner siempre 2 cifras en los números - Agrega 0 cuando sea menor de 10 (2 cifras)
    if (cs<10) {cs="0"+cs;} 
    if (sg<10) {sg="0"+sg;} 
    if (mn<10) {mn="0"+mn;} 
    if (ho<10) {ho="0"+ho;}
...
function parar() { 
    ...
    document.cron.boton2.value="Continue";
}
...
function continuar() {
    ...
    document.cron.boton2.value="Stop"; 
    document.cron.boton1.disabled=false; //activa boton 1 si estuviera desactivado
}
...
function reiniciar() {
    ...
    visor.innerHTML = "00:00:00:00"; //visor a cero
